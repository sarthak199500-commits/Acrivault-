...
/*
 * Extracted from OnboardingScreen.tsx, which owned the only real brand marks in
 * the app as private module-level components. The identity detail panel's
 * source cards do the same job as onboarding's connector cards — name which
 * cloud you are looking at — so they now share this.
 *
 * Note the division of labour with the two neighbouring cloud components, which
 * stay as they are:
 *   - CloudGlyph    monochrome text chip ("AWS"/"AZ"/"GCP"), for tight inline runs
 *   - ProviderBadge coloured dot + name, for dense table rows
 *   - ProviderMark  the real brand logo, where identifying the provider IS the task
 * Full-colour logos are deliberately kept out of the virtualised inventory table:
 * three brand palettes repeated down 1,500 rows would compete with the risk
 * colour that the design reserves as its only carrier of meaning.
 */
 ...
 * Decorative — always pair with the provider name in adjacent text, never rely on
 * the mark alone to convey which cloud this is.
 */
export function ProviderMark({ cloud, className }: { cloud: Cloud; className?: string }) {
  if (cloud === 'aws') {
    return <AwsMark className={cn('h-4 w-auto shrink-0 text-text', className)} />;
  }
  return <img src={PROVIDER_LOGO[cloud]} alt="" className={cn('h-4 w-4 shrink-0', className)} />;
}
